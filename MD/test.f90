module md_module
  implicit none
  integer,parameter:: npart= 1200, niter_eq=100, niter=100, n_calc_av=10
...
contains
  ...
  subroutine init_vel()
    ...
    kinetic_energy=0.5d0*mass*kinetic_energy
  ...
  subroutine update_vel()
    implicit none
    real*8:: av_acc(3*npart)
    kinetic_energy=0.0d0
    av_vx=0.0d0
    av_vy=0.0d0
    av_vz=0.0d0

    av_acc=dtby2bym*(old_force+force)
    do i=1,npart
       vel(3*i-2)=vel(3*i-2)+av_acc(3*i-2)
       vel(3*i-1)=vel(3*i-1)+av_acc(3*i-1)
       vel(3*i)=vel(3*i)+av_acc(3*i)
       av_vx=vel(3*i-2)+av_vx
       av_vy=vel(3*i-1)+av_vy
       av_vz=vel(3*i)+av_vz
       kinetic_energy=kinetic_energy+vel(3*i-2)*vel(3*i-2)+vel(3*i-1)*vel(3*i-1)+vel(3*i)*vel(3*i)
  ...
  subroutine calc_force()
    ...
    do i=1,npart-1
       do j=i+1,npart
          ...
          if (abs(x).gt.llxby2)x=(llx-abs(x))*(-x/abs(x))
          if (abs(y).gt.llyby2)y=(lly-abs(y))*(-y/abs(y))
          if (abs(z).gt.llzby2)z=(llz-abs(z))*(-z/abs(z))
          r=dsqrt(x*x+y*y+z*z)
          if (r<=rc) then
...
program MD
  ...
  !    print*,"total energy",potential_energy+kinetic_energy
